...
`init --features` / `cli add` 按需安装可选基础设施（mysql/redis/metrics 等）；**不包含** log/trace/errors，且无 `features.*` 配置开关。
...
- HTTP 约定见 `.cursor/rules/12-http-routing.mdc`：默认 `/api`（无版本）；path/query/body 分工；仅并行契约时引入 `/api/v2`。
